...
def extract_ft_data(html):
    """Extrae datos de precios de la tabla HTML de Financial Times"""
    try:
        ...
        return [{'date': k, 'close': v} for k, v in seen.items()]
...
def update_from_ft(fund):
    """Actualiza precios desde Financial Times para un fondo"""
    isin = fund['isin']
    ft_code = fund['ft']
    url = f"https://markets.ft.com/data/funds/tearsheet/historical?s={ft_code}"
    
    try:
        print(f"[FT] Fetching {isin}...")
        ...
        if updated > 0:
            print(f"[FT] ✓ Updated {updated} prices for {isin}")
        else:
            print(f"[FT] = No changes for {isin}")
...
def main():
    ...
    print(f"=== Financial Times Update Completed ({success_count}/{len(FUNDS)} successful) ===\n")
